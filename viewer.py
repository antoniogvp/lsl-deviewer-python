...
class Viewer(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, selectedStreams, streams, sampling_rate):
        ...
        for num in selectedStreams:
            ...
            self.inlet.append(StreamInlet(streams[num], 1000))
        ...
        self.configureTimers(sampling_rate)
        
    def configureTimers(self, sampling_rate):
        self.timerData = QtCore.QTimer()
        self.timerData.timeout.connect(self.updateData)
        self.timerData.start(1000.0/sampling_rate)
        ...
        for stream in self.inlet:
            ...
            if self.dataAcquisition is True:
                if timestamps:
                    ...
                    event.append(float(ts[-1]))
                    event.append(str(y[-1,:]))
...
def Start():
    ...
    if(len(listStreams) == 0):
        ...
        if dialog.exec_() and dialog.checkLineEditPattern():
            selectedStreams, sampling_rate = dialog.returnWindowParameters()
            
            if len(selectedStreams) == 0:
                dialog.showErrorNoStreamSelected()
            else:
                v = Viewer(selectedStreams, streams, int(sampling_rate))
                v.show()
        else:
